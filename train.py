...
opts.scheduler_type = opts.scheduler_type  
...
tissue_data = DSRTestDataset(datadir=tissue_dir,fns='/home/hostname/hostname-MTRRVideo/data/tissue_real_index/train1.txt',size=opts.sampler_size3, enable_transforms=True, unaligned_transforms=False, if_align=True,real=True, HW=[256,256], color_match=False)
...
test_data1 = DSRTestDataset(datadir=test_data_dir1, fns='/home/hostname/hostname-MTRRVideo/data/tissue_real_index/eval1.txt', enable_transforms=False, if_align=True, real=True, HW=[256,256], size=opts.test_size[0], color_match=False)
...
if __name__ == '__main__':
    ...
    min_loss = best_val_loss if best_val_loss is not None else float('inf') 
            ...
            with eval_no_dropout(model):
                ...
                for n1 , test_data1 in enumerate(test_pbar):
                    ...
                    if opts.training:
                        ...
                        grid_all = torch.cat([grid_in, grid_out, grid_tgt, grid_cmap], dim=1)  
                        ...
                        grid_all = torch.cat([grid_in, grid_out3, grid_skip, grid_tgt, grid_cmap, grid_fakeR, grid_mask], dim=2)  
                        save_image(grid_all, os.path.join(output_dir6, f'epoch{i}+{total_test_step}+psnr{psnr:.4g}+ssim{ssim:.4g}.png'))
